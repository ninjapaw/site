...
test.describe('Ninja Paws HQ', () => {
  test('renders the public experience and core links', async ({ page }) => {
    ...
    await expect(page.getByRole('link', { name: /Enterprise/ })).toHaveAttribute('href', 'https://github.com/enterprises/ninjapaws');
  ...
  test('filters project cards without a backend', async ({ page }) => {
    ...
    await expect(cards.locator(':visible')).toHaveCount(3);
